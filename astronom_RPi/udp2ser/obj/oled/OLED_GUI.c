                ...
                sFONT* Font,COLOR Color_Background, COLOR Color_Foreground )
{
    ...
    while(xnumber) {
        Num_Array[Num_Bit] = xnumber % 10 + '0';
        Num_Bit++;
        xnumber /= 10;
    }
	...
	if(Nummber > 0){
		...
		GUI_DisString_EN(Xpoint, Ypoint, "0", Font, Color_Background, Color_Foreground );
	} else {
	    GUI_DisString_EN(Xpoint, Ypoint, "-", Font, Color_Background, Color_Foreground );
		GUI_DisString_EN(Xpoint+5, Ypoint, (const char*)pStr, Font, Color_Background, Color_Foreground );
	}
}
